...
## Independent database/security review — PASSED (no CRITICAL/HIGH)
A fresh-context review confirmed all eight focus areas UPHELD: (1) RLS deny-all is correct and the `leads` `revoke` is genuine defense-in-depth against Supabase's default `anon`/`authenticated` grants — and **no anon-key Supabase client exists anywhere** in the codebase (server-side service role only); (2) service-role-only access confirmed; (3) **zero app-written columns missing** (independently re-derived); (4) data types correct (`bigint` estimates are always rounded ints; `timestamptz`/`date` fed ISO strings); (5) every camelCase column quoted; (6) no `UNIQUE(phone)`, and the `deals.neighborhoodId` FK is safe because harvest upserts neighborhoods before deals; (7) all four indexes back real queries (a composite `(neighborhoodId,dealDate)` is a future-scale option only); (8) executes cleanly on a fresh project (`gen_random_uuid()` core built-in, create order satisfies the FK, `anon`/`authenticated` roles exist).

### Non-blocking follow-up (reported, NOT fixed — out of scope for this schema-only wave)
- **`floor` fractional-input validation gap (app-level):** `floor` is correctly typed `int`, but neither the client (`components/ValuationWizard.tsx` `<input type="number">`, no step) nor `app/api/lead/route.ts` `num()` rejects a fractional value. A user typing `2.5` would pass validation and then **fail the insert against the `int` column** (→ `save_failed`, lead lost). The column type is right; the fix is an integer check in `route.ts` — a natural pairing with the Wave 1 lead-reliability work. Not changed here (this wave alters no app behavior).
- Index names carry camelCase but are unquoted (fold to lowercase in the catalog) — purely cosmetic.
